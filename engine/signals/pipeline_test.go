...
func startPipeline(
	...
	cons := consumer.New(client, pub, namespace, symbols, depth, "test-consumer", noopLogger(),
...
func TestPipeline_SingleEventProducesThreeSignals(t *testing.T) {
	_, client := testutil.NewMiniredis(t)
	ns := testutil.UniqueNamespace(t)

	events := testutil.LoadBtcusdtEvents(t)
	testutil.SeedStream(context.Background(), t, client, ns+":l2:BTCUSDT", events[:1])

	startPipeline(t, client, ns, []string{"BTCUSDT"}, 10)

	msgs := waitForSignals(t, client, ns+":signals", 3, 5*time.Second)
...
func TestPipeline_TenEventsProducesThirtySignals(t *testing.T) {
	_, client := testutil.NewMiniredis(t)
	ns := testutil.UniqueNamespace(t)

	events := testutil.LoadBtcusdtEvents(t)
	require.Len(t, events, 10, "seed file must have exactly 10 events")
	testutil.SeedStream(context.Background(), t, client, ns+":l2:BTCUSDT", events)

	startPipeline(t, client, ns, []string{"BTCUSDT"}, 10)

	msgs := waitForSignals(t, client, ns+":signals", 30, 5*time.Second)
	assert.Len(t, msgs, 30, "10 events × 3 signals each = 30")
}

// ── signal values ─────────────────────────────────────────────────────────────

func TestPipeline_MidPriceValue(t *testing.T) {
	_, client := testutil.NewMiniredis(t)
	ns := testutil.UniqueNamespace(t)
...
func TestPipeline_SpreadBpsValue(t *testing.T) {
	_, client := testutil.NewMiniredis(t)
	ns := testutil.UniqueNamespace(t)
...
func TestPipeline_BookImbalanceWithBidPressure(t *testing.T) {
	_, client := testutil.NewMiniredis(t)
	ns := testutil.UniqueNamespace(t)

	// Second event in seed: heavy bid side → imbalance should be positive
	events := testutil.LoadBtcusdtEvents(t)
	testutil.SeedStream(context.Background(), t, client, ns+":l2:BTCUSDT", events[1:2])
...
func TestPipeline_BookImbalanceWithAskPressure(t *testing.T) {
	_, client := testutil.NewMiniredis(t)
	ns := testutil.UniqueNamespace(t)

	// Third event: heavy ask side → imbalance should be negative
	events := testutil.LoadBtcusdtEvents(t)
	testutil.SeedStream(context.Background(), t, client, ns+":l2:BTCUSDT", events[2:3])
...
func TestPipeline_EventTimePropagatesToSignals(t *testing.T) {
	_, client := testutil.NewMiniredis(t)
	ns := testutil.UniqueNamespace(t)
...
func TestPipeline_MultipleSymbols(t *testing.T) {
	_, client := testutil.NewMiniredis(t)
	ns := testutil.UniqueNamespace(t)
	...
	testutil.SeedStream(ctx, t, client, ns+":l2:BTCUSDT", btcEvents[:3])
	...
	msgs := waitForSignals(t, client, ns+":signals", 15, 5*time.Second)
	assert.GreaterOrEqual(t, len(msgs), 15)
}

// ── backtest namespace ────────────────────────────────────────────────────────

func TestPipeline_BacktestNamespace(t *testing.T) {
	_, client := testutil.NewMiniredis(t)
	runID := "test-run-abc-123"
	ns := "erebor:backtest:" + runID

	events := testutil.LoadBtcusdtEvents(t)
	testutil.SeedStream(context.Background(), t, client, ns+":l2:BTCUSDT", events[:2])

	startPipeline(t, client, ns, []string{"BTCUSDT"}, 10)

	// Signals must appear in the backtest-namespaced output stream.
	msgs := waitForSignals(t, client, ns+":signals", 6, 5*time.Second)
	assert.GreaterOrEqual(t, len(msgs), 6)

	// Live stream must remain empty.
	liveSignals, _ := client.XRange(context.Background(), "erebor:live:signals", "-", "+").Result()
	assert.Empty(t, liveSignals, "backtest signals must not leak into live stream")
}

// ── malformed event ───────────────────────────────────────────────────────────

func TestPipeline_MalformedEventIsSkipped(t *testing.T) {
	_, client := testutil.NewMiniredis(t)
	ns := testutil.UniqueNamespace(t)
	ctx := context.Background()
	...
	events := testutil.LoadBtcusdtEvents(t)
	testutil.SeedStream(ctx, t, client, ns+":l2:BTCUSDT", events[:1])
...
func TestPipeline_SignalDepthAffectsImbalance(t *testing.T) {
	events := testutil.LoadBtcusdtEvents(t)
	...
	for _, depth := range []int{1, 5, 10} {
		...
		t.Run(fmt.Sprintf("depth_%d", depth), func(t *testing.T) {
			...
			testutil.SeedStream(context.Background(), t, client, ns+":l2:BTCUSDT", events[4:5])
...
func TestPipeline_BookImbalanceParamsContainDepth(t *testing.T) {
	_, client := testutil.NewMiniredis(t)
	ns := testutil.UniqueNamespace(t)
...
func TestPipeline_RealRedis(t *testing.T) {
	...
	events := testutil.LoadBtcusdtEvents(t)
	testutil.SeedStream(ctx, t, client, ns+":l2:BTCUSDT", events[:5])

	startPipeline(t, client, ns, []string{"BTCUSDT"}, 10)

	msgs := waitForSignals(t, client, ns+":signals", 15, 10*time.Second)
